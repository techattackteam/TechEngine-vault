...
| Sep 5 to Sep 18 | Sprint 05, planned on the Sep 5-6 boundary | **M3** candidate: project root + `project.toml` + `projects/dev/` testbed + the rest of `FileAccess`'s mutating half, `write` having shipped at S4-T7, plus whatever M2 leaves. RNG and crash handler still carry, artifact-less |
